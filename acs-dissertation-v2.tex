...
\begin{definition}\label{def:static_bridge}
    ...
    \end{align}
\end{definition}
We will now derive this result from the dynamic version. Most surveys and papers on the Schrödinger bridge include some form of the derivation that we are about to present, however they skip several steps which may make it inaccessible. 
...
The optimal transport problem in its intuitive form is concerned with the transport of a distribution into another distribution subject to cost $c(\vx,\vy)$. In its discrete histogram based formulation it is refered to as the earth movers distance (EMD) \citep{levina2001earth}, since it effectively moves in between two piles of earth. One of the early applications of EMD was in comparing grey-scale images for the development of a image retrieval system. 
...
We have presented the Schrödinger bridge, as well as an algorithmic framework for solving it. In this chapter, we will be comparing the Schrödinger bridge and IPFP to two different methodologies employed in machine learning for mapping between distributions. 
...
In this section we will briefly define and contrast some of the points of failure of each method based on conceptual observations prior to any experimentation.
...
Stochastic Control      & \xmark & \cmark & \cmark & \xmark & \xmark \\
\bottomrule
\end{tabular}
\end{adjustbox}
...
Finally, we select two toy datasets (one unimodal to unimodal and one unimodal to bimodal) which all methods managed to fit visually and construct a hypothesis test in order to compare the methods against each other in a more statistically sound framework.  We have open sourced the code for the experiment and algorithm \footnote{For our proposed approach the experiments can be found at \url{https://github.com/franciscovargas/SC-IPFP/tree/master/SC_IPFP/torch/final_notebooks}. For the experiments with the method by \cite{pavon2018data} the notebooks can be found at \url{https://github.com/franciscovargas/DataDrivenSchrodingerBridge/tree/master/notebooks}}.
\begin{figure}[t]
    \centering
...
First, we summarize our findings, highlighting the empirical capacities of the 3 approaches we trialed. A summary of the results can be found in Figure \ref{fig:comp_exp}. The main practical milestones we consider when evaluating the methods are:
...
\begin{itemize}
    ...
    \item We found that the method by \cite{ruttor2013approximate} had difficulties in learning drifts whose end point distribution was very sharp (low variance). This needs to be explored further, since optimising the hyperparameters may fix this this issue.  
